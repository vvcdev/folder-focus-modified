...
var FolderFocusModePlugin = class extends import_obsidian2.Plugin {
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      this.addCommand({
        id: 'folder-focus-mode-toggle-forced',
        name: 'Specific Focus',
        callback: () => {
          this.settings.autofocusForced = !this.settings.autofocusForced;
          // Refresh the icon state if focus mode is active
          if (this.focusModeEnabled) {
            const explorers = this.getFileExplorers();
            explorers.forEach(explorer => {
              const button = FolderFocusModePlugin.getFocusButton(explorer);
              if (button) {
                this.setFocusedIconButton(button);
              }
            });
          }
        }
  ...
  setFocusedIconButton(icon) {
    ...
    // Add or remove the specific-focus class based on the setting
    if (this.settings.autofocusForced) {
        icon.classList.add("specific-focus");
    } else {
        icon.classList.remove("specific-focus");
    }
  }
